...
# Complex navigation includes following left wall,
# following right wall and random walk algorithms
# to cover as much space in room as possible
# Three speed levels and two distance thresholds are used
# in order to both move fast and avoid collisions
# todo [0, 0, 1] combination
...
navigation_parameters = {
    ...
    'fast_lin_speed': 10.0,  # m/s
    'mid_lin_speed': 4.5,
    'slow_lin_speed': 0.5,

    'fast_ang_speed': np.pi / 0.3,  # rad/s
    'mid_ang_speed': np.pi / 1.4,
    'slow_ang_speed': np.pi / 3,

    'close_distance_threshold': 0.7,  # m
...
class Navigator:
    ...
    def __init__(self, algorithm='follow_left', fast_lin_speed=1.0, mid_lin_speed=0.5, slow_lin_speed=0.2,
                 fast_ang_speed=np.pi / 2, mid_ang_speed=np.pi / 4, slow_ang_speed=np.pi / 8,
                 close_distance_threshold=0.5, far_distance_threshold=1.5, cycle_threshold=5, pub_topic='/cmd_vel',
                 sub_topic0='/base_scan_0', sub_topic1='/base_scan_1', sub_topic2='/base_scan_2'):
    ...
    def move(self, lin_speed):
        move_forward(self, lin_speed)

    def turn(self, ang_speed):
        return turn_left(self, ang_speed)
    ...
    def follow_left_wall(self):
        ...
        if not any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            ...
            # wall on the left
            self.move_forward_right(lin_speed=self.slow_lin_speed, ang_speed=self.slow_ang_speed)
            ...
            rospy.logwarn("curr_sensors_state_close={}".format(self.curr_sensors_state_close))
            self.turn_right(ang_speed=self.mid_ang_speed)
            self.cycle_counter = 0

        elif self.curr_sensors_state_close == [1, 0, 1]:
            # wall on the right and on the left
            # rospy.logwarn("[1, 0, 1] moving forward")
            self.move_forward(lin_speed=self.slow_lin_speed)
            # turn_left(self, self.slow_ang_speed)
            # self.cycle_counter=0
    ...
    def follow_right_wall(self):
        ...
        if not any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            ...
            # wall on the right
            self.move_forward_left(lin_speed=self.slow_lin_speed, ang_speed=self.slow_ang_speed)
    ...
    def random_navigation(self):
        ...
        if self.cycle_counter >= 5 * self.cycle_threshold:
            self.cycle_counter = -self.cycle_threshold

        if not any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            # no obstacles (like [0,0,0] [0,0,0])
            self.move_forward(lin_speed=self.fast_lin_speed)
            self.cycle_counter += 1

        elif any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            # obstacles far (like [0,1,0] [0,0,0])
            self.move_forward(lin_speed=self.mid_lin_speed)
            self.cycle_counter += 1

        elif self.curr_sensors_state_close == [0, 1, 1] or \
                self.curr_sensors_state_close == [0, 0, 1]:
            # wall on the right
            self.move_forward_left(lin_speed=self.slow_lin_speed, ang_speed=self.mid_ang_speed)

        elif self.curr_sensors_state_close == [1, 1, 0] or \
                self.curr_sensors_state_close == [1, 0, 0]:
            self.move_forward_right(lin_speed=self.slow_lin_speed, ang_speed=self.mid_ang_speed)

        elif self.curr_sensors_state_close == [0, 1, 0] or \
                self.curr_sensors_state_close == [1, 1, 1]:
            # wall on the left or in front or everywhere
            if random.choice(direction_choice) == 'left':
                self.turn_left(ang_speed=5 * self.fast_ang_speed)
            elif random.choice(direction_choice) == 'right':
                self.turn_right(ang_speed=5 * self.fast_ang_speed)
            # twice threshold
            # self.cycle_counter=0
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        rospy.loginfo("curr_sensors_state_close: {}".format(commander.curr_sensors_state_close))
        ...
        else:  # commander.cycle_counter <= 3 * commander.cycle_threshold
