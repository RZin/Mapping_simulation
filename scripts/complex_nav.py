...
navigation_parameters = {
    ...
    'mid_ang_speed': np.pi/1.3,
    'slow_ang_speed': np.pi/4,
    
    'close_distance_threshold': 0.65, # m
...
class Navigator:
    ...
    def follow_left_wall(self):
        ...
        if not any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            ...
            self.move_forward_left(lin_speed=self.fast_lin_speed, ang_speed=self.mid_ang_speed)
        ...
        elif    self.curr_sensors_state_close == [0, 0, 1]:
            # wall on the right and on the left
            self.turn_left(ang_speed=self.mid_ang_speed)
    ...
    def follow_right_wall(self):
        ...
        if not any(self.curr_sensors_state_far) and not any(self.curr_sensors_state_close):
            # no obstacles (like [0,0,0] [0,0,0])
            self.move_forward_right(lin_speed=self.fast_lin_speed, ang_speed=self.mid_ang_speed)
            self.cycle_counter+=1
        ...
        elif    self.curr_sensors_state_close == [1, 0, 0]:
            # wall on the right and on the left
            self.turn_left(ang_speed=self.mid_ang_speed)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if 0 <= commander.cycle_counter <= 2 * commander.cycle_threshold:
        ...
        else: # commander.cycle_counter <= 5 * commander.cycle_threshold
